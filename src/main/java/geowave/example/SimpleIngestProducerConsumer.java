...
public class SimpleIngestProducerConsumer extends SimpleIngest {
	...
	public static void main(
		...
		instance = args[1];
		try {
			geowaveDataStore = CommonSetting.getDataStore();
		}
		...
		System.out.println(
			"Finished ingesting data to namespace: " +
					namespace + " at datastore instance: " + instance);
	}
	...
	protected void generateGrid(final DataStore geowaveDataStore ) {
		...
		for (final SimpleFeature sft : getGriddedFeatures(pointBuilder, -10000)) {
			features.add(sft);
		}
	...
	protected static class FeatureCollection implements Iterator<SimpleFeature> {
		private final BlockingQueue<SimpleFeature> queue = new LinkedBlockingQueue<>(10000);
		public boolean ingestCompleted = false;

		public void add(final SimpleFeature sft) {
			try {
				queue.put(sft);
			}
			catch (final InterruptedException e) {
